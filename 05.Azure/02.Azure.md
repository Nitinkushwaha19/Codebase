...
## Hierarchy Overview
- **Resources**: Basic building blocks (VMs, databases, etc.)
- **Resource Groups**: Groupings of resources; each resource belongs to one group.
- **Subscriptions**: Organize resource groups; separate billing and access control.
- **Management Groups**: Organize subscriptions; apply policies and access across multiple subscriptions.

## Key Points
- Actions on a resource group affect all its resources.
- Subscriptions allow separate billing and access policies.
- Management groups enable unified governance and can be nested up to 6 levels.
- Each management group/subscription has only one parent.
- Tenant Root Group is the top-level management group.

## Use Cases
- Separate environments (dev, test, prod) with different subscriptions.
- Apply policies (e.g., VM location restrictions) across multiple subscriptions.
- Grant access to multiple subscriptions at once via management groups.
